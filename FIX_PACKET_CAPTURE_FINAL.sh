...
BACKUP="/home/youssef-amr/soc_project_BACKUP_PACKET_FINAL_$TS"
cp -r /home/youssef-amr/soc_project "$BACKUP"

cat > packet_capture_fix.js <<'JS'
(function () {
...
for f in index.html frontend/index.html monitor/frontend/index.html; do
  ...
  if [ "$dir" != "." ]; then
    cp packet_capture_fix.js "$dir/packet_capture_fix.js"
  fi

  if ! grep -q "packet_capture_fix.js" "$f"; then
    sed -i 's#</body>#<script src="packet_capture_fix.js"></script>\n</body>#i' "$f"
  fi
done

python3 update_packet_capture.py 2>/dev/null || true
