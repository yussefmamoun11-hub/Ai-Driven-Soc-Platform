...
BKP="backups/frontend_conflict_fix_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BKP"
...
# احذف أي حاجة بعد </html>
if "</html>" in s:
    s=s[:s.index("</html>")+len("</html>")]

# عطّل السكريبتات المتعارضة
remove = [
...
# أضف سكريبت واحد فقط قبل </body>
if '<script src="soc_nexus_stable_single_controller.js"></script>' not in s:
    s=s.replace("</body>", '<script src="soc_nexus_stable_single_controller.js"></script>\n</body>')

p.write_text(s)
PY

cat > soc_nexus_stable_single_controller.js <<'JS'
(function(){
...
echo "✅ Frontend conflicts fixed. Only one controller active."
echo "Backup: $BKP/index.html.bak"
