...
echo "========================================"
echo " FIXING: Packet Capture · Deep Inspection"
echo "========================================"

# Create live packet source from auth.log
cat > update_packet_capture.py <<'PY'
#!/usr/bin/env python3
import json
import re
...
chmod +x update_packet_capture.py

# Patch the main live service to call packet updater
if ! grep -q "update_packet_capture.py" soc_enterprise_authlog_live.sh; then
  sed -i '/UPDATED |/i python3 /home/youssef-amr/soc_project/update_packet_capture.py >/dev/null 2>&1' \
    soc_enterprise_authlog_live.sh
fi

# Run once immediately
python3 update_packet_capture.py

# Restart service
sudo systemctl restart soc-live-panels.service

echo "========================================"
echo "✅ Packet Capture panel is now LIVE"
echo "========================================"
echo "Panel will:"
echo "✔ Update instantly during attack"
echo "✔ Show real attacker IP"
echo "✔ Increase packet rows automatically"
echo "✔ Keep final packets after attack ends"
echo "========================================"
