...
echo "[1] Backup..."
mkdir -p backups/unified_8080_$(date +%Y%m%d_%H%M%S)
BKP=$(ls -td backups/unified_8080_* | head -1)
cp -a index.html "$BKP/index.html.bak" 2>/dev/null || true
cp -a script.js "$BKP/script.js.bak" 2>/dev/null || true

echo "[2] Installing dependencies..."
source venv/bin/activate
pip install flask flask-socketio eventlet >/dev/null

echo "[3] Creating unified 8080 SIEM server..."
cat > soc_unified_8080_live.py <<'PY'
#!/usr/bin/env python3
...
chmod +x soc_unified_8080_live.py

echo "[4] Injecting frontend WebSocket layer..."
cat >> index.html <<'HTML'
...
echo "sudo ./START_UNIFIED_8080_LIVE_SIEM.sh"
