...
cat > packet_capture_dashboard.py <<'PY'
#!/usr/bin/env python3
import re, json
...
chmod +x packet_capture_dashboard.py

sudo tee /etc/systemd/system/soc-packet-dashboard.service > /dev/null <<SERVICE
[Unit]
Description=Standalone SOC Packet Capture Dashboard
...
ExecStart=/usr/bin/python3 $PROJECT/packet_capture_dashboard.py
Restart=always
RestartSec=2
...
sudo systemctl enable soc-packet-dashboard.service
