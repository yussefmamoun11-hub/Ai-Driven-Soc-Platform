...
for f in index.html frontend/index.html monitor/frontend/index.html; do
  [ -f "$f" ] || continue
  dir=$(dirname "$f")
  [ "$dir" != "." ] && cp force_packet_capture_dom_v2.js "$dir/force_packet_capture_dom_v2.js"

  if ! grep -q "force_packet_capture_dom_v2.js" "$f"; then
    sed -i 's#</body>#<script src="force_packet_capture_dom_v2.js"></script>\n</body>#i' "$f"
  fi
done
